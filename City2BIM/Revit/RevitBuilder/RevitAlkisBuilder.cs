...
namespace City2BIM.RevitBuilder
{
    class RevitAlkisBuilder
    {
        ...
        public void CreateTopo(List<AX_Object> topoObjs)
        {
            ...
            if (NAS2BIM_prop.DrapeBldgsOnTopo || NAS2BIM_prop.DrapeParcelsOnTopo || NAS2BIM_prop.DrapeUsageOnTopo)
            {
                using (FilteredElementCollector collector = new FilteredElementCollector(doc))
                {
                    ...
                    if (!topos.Contains(Revit_Prop.TerrainId))
                    {
                        NAS2BIM_prop.DrapeBldgsOnTopo = false;
                        NAS2BIM_prop.DrapeParcelsOnTopo = false;
                        NAS2BIM_prop.DrapeUsageOnTopo = false;
                    }
                }
            }
            //----------------
            ...
            foreach (var topoGr in groupedTopo)
            {
                ...
                if (topoGr.Key == AX_Object.AXGroup.building)
                    if (NAS2BIM_prop.DrapeBldgsOnTopo)
                        createTopoPlane = false;

                if (topoGr.Key == AX_Object.AXGroup.parcel)
                    if (NAS2BIM_prop.DrapeParcelsOnTopo)
                        createTopoPlane = false;

                if (topoGr.Key == AX_Object.AXGroup.usage)
                    if (NAS2BIM_prop.DrapeUsageOnTopo)
                        createTopoPlane = false;

                var topoId = Revit_Prop.TerrainId;
        ...
        private List<XYZ> GetTopoPts(IEnumerable<List<C2BPoint[]>> segments, AX_Object.AXGroup axGroup, out double zOffset)
        {
            ...
            {
                var unprojectedPt = GeorefCalc.CalcUnprojectedPoint(new C2BPoint(pt.x, pt.y, zOffset), true);
        ...
        private void CreatePlanViews(double planElev, AX_Object.AXGroup axGroup)
        {
            ...
            bool alreadyExistent
              = (from ViewPlan f in collector2
                 where (f.ViewType == ViewType.FloorPlan && !f.IsTemplate && f.ViewName.Equals("Plan_ALKIS_" + axGroup))
                 select f).Any();
                ...
                {
                    {
                        ...
                        using (Level level1 = Level.Create(doc, planElev))
                        {
                            ...
                            {
                                floorView.ViewName = "Plan_ALKIS_" + axGroup;
