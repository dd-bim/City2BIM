        ...
        private List<int> CalculateVertexCoords(string id, List<C2BPoint> polygon, bool exterior, ref C2BPoint normal, ref C2BPoint centroid)
        {
            ...
            for (int i = 1; i < polygon.Count; i++)
            {
                ...
                for (int j = 0; j < Vertices.Count; j++)
                {
                    ...
                    if (dist < City2BIM_prop.EqualPtSq)
                    {
                        currentInt = j;
            ...
            for (var i = 0; i < Edges.Count - 1; i++)
            {
                for (var j = i + 1; j < Edges.Count; j++)
                {
                    ...
                    if (Edges[i].Start == Edges[j].End && Edges[i].End == Edges[j].Start &&
                        distNorm < City2BIM_prop.EqualPlSq)
                    {
                        try
                        {
                        ...
                        for (var i = 0; i < ordPlanes.Count - 2; i++)
                        {
                            for (var j = i + 1; j < ordPlanes.Count - 1; j++)
                            {
                                for (var k = j + 1; k < ordPlanes.Count; k++)
                                {
                                    ...
                                    if (ctCurrV > 2)
                                        break;          //case if more at least 3 planes are concave at vertex  --> forced order neccessary

                                    if ((ctCurrV + ctAnyV) == 3)
                                        break;

                                    if ((ctCurrV + ctAnyV) > 3 && d < City2BIM_prop.MaxDevPlaneCutSq)
                                        break;

                                    if ((ctCurrV + ctAnyV) > 3 && l == (ctCurrV + ctAnyV))
                                        break;                  // if not breaked yet, break if all concave planes are investigated

                                    if (d < City2BIM_prop.MaxDevPlaneCutSq)
                                        break;                  //break independent of concave plane numbers for fast flow
                                }
